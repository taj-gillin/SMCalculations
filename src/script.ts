...
for (let i = 0; i < Wheel1.length; i++) {
    for (let j = 0; j < Wheel2.length; j++) {
        for (let k = 0; k < Wheel3.length; k++) {
            Combinations.push([Wheel1[i], Wheel2[j], Wheel3[k]]);
        }
    }
}

let PayoffOccurances = {
    "1Cherry": 0,
    "2Cherries": 0,
    "3Cherries": 0,
    "3Oranges": 0,
...
let AveragePayoff = Payoff["1Cherry"] + Payoff["2Cherries"] + Payoff["3Cherries"] + Payoff["3Oranges"] + Payoff["3Watermelons"] + Payoff["3Bells"] + Payoff["3Bars"] + Payoff["3SingleBars"] + Payoff["3DoubleBars"] + Payoff["3TripleBars"];
